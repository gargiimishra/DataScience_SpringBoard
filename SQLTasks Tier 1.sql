...
in question. */
SELECT name as facility_name, IF(monthlymaintenance<100, 'cheap', 'expensive') as category 
FROM `Facilities` 
...
FROM 
(SELECT B.memid,F.name FROM Bookings as B 
 JOIN Facilities as F ON B.facid = F.facid AND F.name like 'Tennis Court%')  AS BF 
JOIN Members AS M 
ON BF.memid = M.memid
